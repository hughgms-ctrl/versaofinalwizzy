...
import { Bot, Trash2, Sparkles, ArrowRight, FolderInput, Workflow } from 'lucide-react';
...
// Sempre um cartão de "Agente" simples e reaproveitável -- nunca vira
// "Orquestração", mesmo quando usado dentro de uma ou várias orquestrações
// (ver conversa com o usuário: "o agente simples fica quietinho lá... ele
// pode ser usado em quantas orquestrações eu quiser"). `usageCount` só dá um
// sinal discreto de quantas orquestrações o usam hoje.
export function AgentListItem({ agent, folders = [], onMoveToFolder, usageCount = 0 }: AgentListItemProps) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                {usageCount > 0 && (
                  <span className="flex items-center gap-1 text-xs text-violet-500/80 ml-1">
                    <Workflow className="h-3 w-3" />
                    usado em {usageCount} orquestraç{usageCount > 1 ? 'ões' : 'ão'}
                  </span>
                )}
              </div>
            </div>
