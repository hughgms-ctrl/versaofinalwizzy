...
export default function IntegrationsPage() {
  ...
  const handleSave = () => {
    // Clear feature overrides that are disabled
  ...
  const resolveProvider = (feature: AIFeature): AIProvider => {
    if (featureOverrides[feature]) {
      const override = (formData as any)[`${feature}_provider`] as AIProvider | null;
      if (override) return override;
    }
    return formData.ai_provider || 'lovable';
  };

  const resolveModel = (feature: AIFeature): string => {
    if (featureOverrides[feature]) {
      const model = (formData as any)[`${feature}_model`] as string | null;
      if (model) return model;
    }
    return formData.default_model || 'google/gemini-3-flash-preview';
  };

  if (isLoading) {
    return (
      <MainLayout title="Integrações" subtitle="Gerencie conexões e provedores de IA">
  ...
  return (
    <MainLayout title="Integrações" subtitle="Configure provedores de IA e API Keys">
        ...
        <Card className="bg-card border-border">
          ...
          <CardContent className="space-y-4">
            {(Object.keys(FEATURE_LABELS) as AIFeature[]).map((feature) => {
              ...
              const currentProvider = resolveProvider(feature);
              const currentModel = resolveModel(feature);
