...
- [~] **6D — Paginação real (keyset)** em `useMessages` (`.limit(50)` + cursor `created_at`) e `useContacts` (`.range()` + virtualização). Garantir que o frontend use **cursor**, não OFFSET. *(dividida em 6D-i chat e 6D-ii contatos)*
  ...
  - [ ] **6D-ii — contatos (`useContacts`).** Decisão do dono: **cap server-side (últimos N) + virtualização**, mantendo busca/tag/data atuais dentro do conjunto.
