...
- [~] **3C — `process-scheduled-messages` (N+1, cron 1 min).**
  ...
  > ⚠️ **Drift adicional do banco (2026-06-16):** o índice ANTIGO `idx_conversations_contact_org_unique (contact_id, organization_id)` **ainda existe** no banco vivo — a migration `20260610173000` deveria tê-lo dropado ao introduzir o escopo por instância (`idx_conversations_contact_org_instance_unique`). Efeito: **escopo por instância NÃO está ativo** (1 conversa por contato+org). O `process-scheduled-messages` foi ajustado para **reutilizar a conversa existente** (busca por contato+org, preferindo a instância agendada) em vez de tentar criar outra. **Decisão pendente (fora da Fase 3):** alinhar o banco ao repo (dropar o índice antigo p/ ativar escopo por instância) — afeta RLS/dedup, avaliar em janela própria.
