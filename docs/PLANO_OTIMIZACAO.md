...
- [x] **3C — `process-scheduled-messages` (N+1, cron 1 min).** ✅ **Validado em 2026-06-16** — agendamento novo (single) enviado e gravado ponta a ponta (msg em `messages` + entregue no WhatsApp), após aplicar a migration do trigger `auto_assign_workspace`. Deviations abaixo permanecem.
  ...
  > - **Etapa 1 — migration `20260616124000_activate_conversation_instance_scoping.sql`** (garante índice por instância + dropa o legado). Ativa 2-chats e corrige o recebimento. Sem janela de perda: até aplicar, os fallbacks tolerantes seguram; depois de aplicar, o insert por instância passa a criar o 2º chat normalmente.
  > - **Etapa 2 (higiene, após validar)** — reverter os fallbacks cross-instância que adicionei (`zapi-webhook.findOrCreateConversation` e `process-scheduled-messages.preloadConversations`) para voltarem a ser **scoped por instância** (viram código morto após a Etapa 1; remover p/ clareza).
