...
- [x] **Fase 0** — Preparação e baseline *(branch + mecanismo de deploy OK. **Baseline "antes" não foi capturado e as mudanças já estão no ar → comparativo antes×depois irrecuperável.** Em vez disso, **health check do estado atual** feito em 2026-06-22 — ver `docs/health_check_pos_otimizacao.md`.)*
- [⚠️] **Fase 1** — Quick wins: filtros de Realtime + RLS `(select auth.uid())` *(1A frontend ✅. **1B foi aplicado e validado em 2026-06-15, mas REVERTEU**: o health check de 2026-06-22 achou ~100 políticas de volta com `auth.uid()` nu. **Causa: o Lovable re-aplica as políticas RLS base (nuas) a cada sync, sobrescrevendo mudanças manuais do SQL Editor** (objetos aditivos sobrevivem; RLS não). **Impacto baixo** (perf em escala; zero efeito em segurança/isolamento; imperceptível no volume atual). **Decisão: não reaplicar via SQL Editor** (reverteria de novo); revisitar **via Lovable** se o volume crescer. Detalhe em `docs/health_check_pos_otimizacao.md`.)*
