...
- 🔨 **chat-media + flow-media (write scope) — CÓDIGO PRONTO, aguardando aplicar+deploy+teste** (2026-07-14). Migration `20260714130000_storage_chat_flow_media_write_scope.sql`: buckets SEGUEM PÚBLICOS (leitura é requisito do provedor); INSERT/UPDATE trocados de `authenticated`/bucket_id para `foldername[1] = get_user_org_id(auth.uid())`. Front: TODOS os 6 uploads passam a prefixar `${orgId}/...` — useMediaUpload (param orgId, caller ConversationDetail passa profile.organization_id; teste atualizado), ProfilePage (avatar `${orgId}/avatars/...`), Create+EditScheduledMessageDialog (`${orgId}/scheduled/${workspace}/...`, useAuth adicionado), NodePropertiesPanel/MediaUploadField (`${orgId}/${type}s/...`) e RemarketingStepsEditor (`${orgId}/followup-media/...`). Uploads via edge (service_role) NÃO mudam (imunes a RLS). Arquivos antigos legíveis (read público) → sem migração. Typecheck + teste do hook verdes.
- ⏳ contact-files — ADIADO (sessão dedicada, alto risco — ver seção acima).
