...
- 🔨 **contact-files — FASE A (leitores → signed URL/download) CÓDIGO PRONTO, aguardando deploy+validação** (2026-07-15). Estratégia escolhida com o usuário: **privatização completa** (fecha READ+WRITE), executada em duas fases — **Fase A: converter TODOS os leitores (backward-compatible, bucket ainda público)**; **Fase B: flip `public=false` + policies (com janela de teste)**. Fase A feita:
  ...
  - **FASE B (flip) — NÃO iniciada.** Precisa: migration `public=false` + SELECT org-scoped (contact_files JOIN contacts; template-logos/document-images por `${orgId}`), INSERT/UPDATE escopados, DELETE já escopado. **Ressalva quiz:** o `folder[1]` de `quiz-uploads/${quizId}/...` é literalmente `quiz-uploads` (não um `contact_id`), então a policy SELECT via JOIN `contacts` por `folder[1]=contact_id` NÃO cobre esses arquivos → adicionar policy SELECT dedicada pro prefixo `quiz-uploads/` (JOIN em `contact_files.storage_path` → org) + manter a policy anon INSERT existente. **Readers a converter no MESMO deploy do flip:** TemplateEditor `<img>` lateral + camada de reescrita display-only do `content_html` (editor + previews FillForm). Deploy com janela de teste (assinar contrato ponta a ponta, verificar público, CRM, gerar doc, quiz, exibir template com logo/imagem embutida).
