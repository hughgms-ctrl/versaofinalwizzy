...
- ✅ **contact-files — FASE A (leitores → signed URL/download) VALIDADA EM PROD + PUSHED** (2026-07-15, commit `098ca41d`). Estratégia escolhida com o usuário: **privatização completa** (fecha READ+WRITE), executada em duas fases — **Fase A: converter TODOS os leitores (backward-compatible, bucket ainda público)**; **Fase B: flip `public=false` + policies (com janela de teste)**. Testado em prod pelo usuário (CRM, fluxos públicos de assinatura/verificação, assinar contrato ponta a ponta, docs autenticados) — verde. Fase A feita:
  ...
  - ⏳ **FASE B (flip) — CÓDIGO ESCRITO (2026-07-15), aguardando deploy coordenado + janela de teste.** Decisões alinhadas com o usuário: imagens embutidas do `content_html` → **assinar só na exibição** (load-sign / save-strip, honra a A.5); drift de policies → **DO block defensivo**. Escrito:
    ...
    - **Typecheck do front limpo (0 erros).**
  - ✅ **FASE B (flip) VALIDADA EM PROD + COMMITADA/PUSHADA (2026-07-18).** Deploy coordenado feito (migration no SQL Editor + bundle front/edge juntos). Checklist da janela de teste passou: assinar contrato ponta a ponta; verificação pública; CRM (abrir/baixar/deletar arquivo; arquivo de quiz num contato); doc autenticado (download+ZIP); PublicFormPage (PDF anon + WhatsApp); template com logo+imagem embutida; cross-org 403.
    - **3 bugs pescados na janela de teste — todos corrigidos+deployados+validados:**
      1. **Upload de quiz falhava (RLS violation):** name-shadowing na policy `20260709120500` — `(storage.foldername(name))[2]` NÃO-qualificado dentro de `EXISTS ... FROM quizzes q` resolvia p/ `q.name` (título do quiz) em vez de `objects.name` (path) → foldername(título)[2]=NULL → EXISTS sempre falso → todo upload anon rejeitado. Uploads antigos funcionaram porque a policy era a versão simples; a endurecida bugada entrou depois. FIX = migration `20260718120000_fix_quiz_upload_policy_name_shadowing.sql` (qualifica `objects.name`). MESMA classe do bug de task-files (20260713120000) — reforça: SEMPRE qualificar `objects.name` quando a policy joina tabela com coluna `name`.
      2. **Arquivo de quiz não linkava ao contato (pré-existente, não é o flip):** `PublicQuizPage` fazia `onAnswer(fileValue, d.variable)`; sem variável no bloco, o arquivo caía em `answers` mas não em `variables`, e `quiz-actions` linka varrendo só `variables` (front nunca manda `answers`) → órfão no storage. FIX front-only: chave sintética `__file_${block.id}` quando não há variável + rótulo "Arquivo" no `buildQuizSummary`.
      3. **"Abrir em nova aba" abria aba EM BRANCO:** `window.open('', '_blank', 'noopener,noreferrer')` retorna `null` por spec com `noopener` → perde a referência da janela → `win.location.href` pós-await nunca roda. FIX em 2 helpers (`ContactFilesSection.openContactFileInNewTab` + `documentFiles.openDocFileInNewTab`): abrir sem noopener + `win.opener = null`.

## STATUS FINAL (2026-07-18): GAP DE STORAGE FECHADO
Todos os buckets tratados e validados em prod. carousel-images/contact-avatars → privados; task-files → privado (piloto); contact-files → privado (Fase A+B); chat-media/flow-media → públicos DE PROPÓSITO (leitura é requisito do provedor de WhatsApp) com write escopado por org. Ver memória `storage-buckets-open-rls-gap`.
