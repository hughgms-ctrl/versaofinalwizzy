...
- 🔨 **contact-files — FASE A (leitores → signed URL/download) CÓDIGO PRONTO, aguardando deploy+validação** (2026-07-15). Estratégia escolhida com o usuário: **privatização completa** (fecha READ+WRITE), executada em duas fases — **Fase A: converter TODOS os leitores (backward-compatible, bucket ainda público)**; **Fase B: flip `public=false` + policies (com janela de teste)**. Fase A feita:
  ...
  - ⚠️ **A.5 (achado na varredura, PENDENTE antes do flip):** leitores extras de contact-files: TemplateEditor `<img>` logo (`${orgId}/template-logos`), RichTextEditor imagens embutidas (`${orgId}/document-images`), PublicQuizPage arquivos de resposta (`quiz-uploads/${quizId}`). Os dois primeiros usam prefixo `${orgId}` → policy SELECT org-scoped por pasta + assinatura client-side. Quiz precisa de decisão.
  - **FASE B (flip) — NÃO iniciada.** Precisa: migration `public=false` + SELECT org-scoped (contact_files JOIN contacts; template-logos/document-images por `${orgId}`), INSERT/UPDATE escopados, manter policy anon `quiz-uploads` + DELETE já escopado. Deploy com janela de teste (assinar contrato ponta a ponta, verificar público, CRM, gerar doc, quiz).
