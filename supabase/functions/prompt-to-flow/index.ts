...
10d. "action-query-contacts" - Pergunta algo sobre a BASE de contatos da organizacao (nao sobre o contato da conversa). Data: { label, queryMode: "count"|"list"|"group", filters: [{ id, type: "tag"|"pipeline"|"custom_field", negate?, tagId?, pipelineId?, columnId?, fieldKey?, operator?: "equals"|"not_equals"|"contains"|"is_empty"|"is_not_empty", value? }], listFields: ["name"|"phone"|"email"|key de campo personalizado], listLimit, groupByField?, groupExpectedValues?: string[], groupIncludeEmpty?: boolean, outputVariable }
    ...
    - queryMode "count" grava o numero em {{outputVariable}}; "list" grava um texto com uma linha por contato; "group" grava uma linha "valor | contagem" por valor do campo em groupByField. Nos tres casos {{outputVariable}_total} traz um total (no "group", a soma das linhas).
