...
10d. "action-query-contacts" - Pergunta algo sobre a BASE de contatos da organizacao (nao sobre o contato da conversa). Data: { label, queryMode: "count"|"list"|"group", filterLogic?: "and"|"or", filters: [{ id, type: "tag"|"pipeline"|"custom_field", negate?, tagId?, pipelineId?, columnId?, fieldKey?, operator?: "equals"|"not_equals"|"contains"|"is_empty"|"is_not_empty", value? }], listFields: ["name"|"phone"|"email"|key de campo personalizado], listLimit, groupByField?, groupExpectedValues?: string[], groupIncludeEmpty?: boolean, outputVariable }
    - filterLogic ausente ou "and": todos os filtros precisam bater ao mesmo tempo. Use "or" quando o pedido for "quem esta na coluna A OU na coluna B", "quem tem a tag X ou a tag Y" -- sem ele, dois filtros viram uma intersecao que quase sempre devolve zero. No modo "or" nao existe negate: o motor recusa o no.
    - Sem filtro, conta a organizacao inteira.
    - queryMode "count" grava o numero em {{outputVariable}}; "list" grava um bloco por contato (nome como titulo e uma linha "campo: valor" por campo preenchido); "group" grava uma linha "valor | contagem" por valor do campo em groupByField. Nos tres casos {{outputVariable}_total} traz um total (no "group", a soma das linhas).
    - Use "group" quando o pedido for "quantos escolheram cada opcao", "distribuicao por", "um grafico de". A saida ja sai no formato que o gerador de PDF le dentro de [[GRAFICO titulo]] -- nao monte um no de contagem por opcao possivel. groupByField precisa ser a chave de um campo personalizado que EXISTA; groupExpectedValues fixa a ordem e faz as opcoes com zero aparecerem.
