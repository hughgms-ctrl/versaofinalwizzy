...
export async function calculateOrganizationUsage(
  ...
  const [profiles, workspaces, instances, conversations] = await Promise.all([
    ...
    safeSelect(client, 'whatsapp_instances', 'id, organization_id, is_active, status, phone_number, connected_at'),
  ...
  for (const instance of instances || []) {
    ...
    const isWhatsAppNumberInUse = instance.status === 'connected' ||
      instance.is_active ||
      instance.connected_at ||
      instance.phone_number
    if (isWhatsAppNumberInUse) usageByOrg[instance.organization_id].active_instances += 1
  }
