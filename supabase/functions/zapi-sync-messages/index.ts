...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // UAZAPI: GET /messages/{phone}?limit=N
    const url = `${uazapiBaseUrl}/messages/${contactPhone}?limit=${amount}`;
    console.log(`Fetching ${amount} messages for conversation ${conversationId}...`);
    ...
    console.log(`Got ${messages.length} messages from UAZAPI`);
