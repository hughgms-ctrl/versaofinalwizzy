...
-- Único read direto do client (verificado na fonte 2026-07): a Sidebar lê
-- 'show_client_plans_menu' (usuário autenticado). 'allow_signups' pode ser lido
-- na tela de signup. Todo o resto (payment_gateway_settings,
-- whatsapp_connection_settings, whatsapp_provider_strategy, entry_flow_*) é
-- sensível e só deve ser lido por:
...
CREATE POLICY "Public can read non-sensitive settings"
  ...
  FOR SELECT
  USING (key IN ('allow_signups', 'show_client_plans_menu'));
